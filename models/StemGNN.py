...
class StockBlockLayer(nn.Module):
    ...
    def forward(self, x, mul_L):
        ...
        # x: [batch_size, seq_len, feature] -> [batch_size, 1, feature, seq_len]
        x = x.unsqueeze(1)
...
class StemGNN(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.unit = kwargs.get('input_size', 1)
        self.output_size = kwargs.get('output_size', 1)
        self.stack_cnt = kwargs.get('stack_cnt', 2)
        self.alpha = kwargs.get('leaky_rate', 2)
        self.time_step = kwargs.get('history_len', 2)
        self.dropout_rate = kwargs.get('dropout_rate', 2)
        self.multi_layer = kwargs.get('multi_layer', 5)
        ...
        self.out_linear = nn.Linear(self.unit, self.output_size)
    ...
    def cheb_polynomial(self, laplacian):
        ...
        laplacian = laplacian.unsqueeze(0) # [1, N, N]
    ...
    def self_graph_attention(self, input):
        ...
        # query shape [batch, N, 1]
        # torch.repeat(1,1,n) means repeat the first dimension 1 times, the second dimension 1 times, the third dimension n times
        data = key.repeat(1, 1, N).view(bat, N * N, 1) + query.repeat(1, N, 1)
        # data shape [batch, N*N, 1]
        data = data.squeeze(2)
        data = data.view(bat, N, -1)
        data = self.leakyrelu(data)
        attention = F.softmax(data, dim=2)
        attention = self.dropout(attention)
        # attention shape [batch, N, N]
        return attention
    ...
    def forward(self, x):
        ...
        # x: [batch, seq_len, feature] -> [batch, 1, feature, seq_len]
        X = x.unsqueeze(1).permute(0, 1, 3, 2).contiguous()
        result = []
        for stack_i in range(self.stack_cnt):
            # x shape [batch, 1, feature, seq_len] (32, 1, 14, 12)
            #mul_L shape [K, N, N]
            forecast, X = self.stock_block[stack_i](X, mul_L)
            result.append(forecast)
        forecast = result[0] + result[1]
        forecast = self.fc(forecast).squeeze()
        forecast = self.out_linear(forecast).unsqueeze(1)
        return forecast.contiguous()#, attention
